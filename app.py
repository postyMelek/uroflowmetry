...
[data-testid="stSidebar"] label {
    color: #c8d8f0 !important; font-size: 0.78rem !important;
...
[data-testid="stSidebar"] .stNumberInput > div > div > input {
    ...
    color: #f0f6ff !important; font-family: var(--font-body) !important; font-size: 0.875rem !important;
}
[data-testid="stSidebar"] .stSelectbox > div > div {
    background: rgba(255,255,255,0.10) !important;
    border: 1px solid rgba(255,255,255,0.20) !important;
    border-radius: var(--radius-sm) !important; color: #f0f6ff !important;
}
...
.stNumberInput > div > div > input {
    ...
    font-family: var(--font-body) !important; font-size: 0.875rem !important; transition: border-color 0.2s;
}
...
label {
    ...
    font-weight: 600 !important; color: var(--text-secondary) !important; letter-spacing: 0.01em !important;
}
...
[data-testid="stFileUploader"]:hover { border-color: var(--accent-mid); background: var(--accent-light); }

.stButton > button {
    font-family: var(--font-body) !important; font-weight: 700 !important;
    letter-spacing: 0.04em !important; border-radius: var(--radius) !important; transition: all 0.2s ease !important;
}
.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #1a5fa8, #2176c7) !important;
    border: none !important; color: white !important; padding: 0.75rem 2rem !important;
    box-shadow: 0 4px 14px rgba(26,95,168,0.30) !important;
}
.stButton > button[kind="primary"]:hover {
    background: linear-gradient(135deg, #2176c7, #3b93e0) !important;
    transform: translateY(-1px) !important; box-shadow: 0 6px 20px rgba(26,95,168,0.38) !important;
}
.stButton > button[kind="primary"]:disabled {
    background: #b0bdd0 !important; box-shadow: none !important; transform: none !important;
}
...
""", unsafe_allow_html=True)
    ...
    st.markdown("""<style>.stCheckbox label p { color: white !important; font-weight: 500; }</style>""", unsafe_allow_html=True)
...
<div style="background:#ffffff; border:1.5px solid #d1d9e6; border-radius:18px;
     padding:2rem 2.5rem; margin-bottom:1.8rem; box-shadow:0 4px 16px rgba(26,42,67,0.08);">
        ...
        with st.spinner("Mengekstrak waveform..."):
            if dta_file is not None:
                with tempfile.NamedTemporaryFile(suffix='.dta', delete=False) as tmp:
                    tmp.write(dta_file.read()); tmp_path = tmp.name
                wave = extract_from_dta(tmp_path)
                os.unlink(tmp_path)
                if wave is not None:
                    waveform_data = wave; waveform_source = "DTA"; waveform_ready = True

            if pdf_file is not None:
                with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as tmp:
                    tmp.write(pdf_file.read()); tmp_path = tmp.name
                wave_pdf, ufm_from_pdf = extract_from_pdf(tmp_path)
                os.unlink(tmp_path)
                if waveform_data is None and wave_pdf is not None:
                    waveform_data = wave_pdf; waveform_source = "PDF"; waveform_ready = True
...
""", unsafe_allow_html=True)
        ...
        fig.patch.set_facecolor('#f5f7fa'); ax.set_facecolor('#eef1f6')
...
<div style="background:#f5f7fa; border:2px dashed #d1d9e6; border-radius:10px;
     ...
     letter-spacing:0.1em; margin-top:0.8rem;">AWAITING FILE UPLOAD</div>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    # ── FIX: value=None bukan value=0 ────────────────────────────────────────
    # Nilai 0 membuat scaler menghasilkan output ekstrem → bias prediksi 100% BOO
    # None → imputer isi dengan median training data (~40ml) → aman
    vol_prostat_raw = st.number_input(
        "Volume Prostat (ml/USG)",
        min_value=1, max_value=300,
        value=None,          # ← KUNCI: None bukan 0
        step=1,
        placeholder="Isi jika ada hasil USG prostat"
    )
    vol_prostat = float(vol_prostat_raw) if vol_prostat_raw is not None else np.nan
    ...
    if ipp_present:
        ipp_value = st.number_input(
            ...
            value=None,      # ← KUNCI: None bukan 0.0
            step=0.1, format="%.1f",
            placeholder="mis: 5.0"
        )
...
""", unsafe_allow_html=True)
    ...
    cli_preview = {
        ...
        "Usia":            float(usia),
        "Volume Prostat":  vol_prostat,   # sudah float atau NaN
...
""", unsafe_allow_html=True)
    ...
    # Tip jika volume prostat belum diisi
    if np.isnan(vol_prostat) and route == 'C':
        st.markdown('<div style="background:#e8f1fb; border:1.5px solid #b8d2f0; border-radius:10px; padding:0.6rem 0.9rem; font-size:0.75rem; color:#1a5fa8; margin-top:0.5rem;">📌 Isi Volume Prostat dari hasil USG untuk meningkatkan akurasi.</div>', unsafe_allow_html=True)
...
if run_analysis and waveform_ready:
    ...
    clinical_full = {
        ...
        "Skor Voiding":    float(sc_voiding), "QoL": float(sc_qol),
        "Usia":            float(usia),
        "Volume Prostat":  vol_prostat,   # float atau NaN — tidak pernah 0
        "IPP":             float(ipp_value) if ipp_present and ipp_value is not None else np.nan,
        "Diabetes":        1.0 if diabetes else 0.0,
        "Stroke":          1.0 if stroke else 0.0,
        "Neurology abnormalities": 1.0 if neurologi else 0.0
    ...
    conf_bg = {"TINGGI":"#dcfce7","SEDANG":"#fef3c7","RENDAH":"#fdecea"}[conf]
    conf_bd = {"TINGGI":"#86efac","SEDANG":"#fcd34d","RENDAH":"#f5c6c2"}[conf]
    conf_cl = {"TINGGI":"#166534","SEDANG":"#b45309","RENDAH":"#c0392b"}[conf]
    ...
    if result['needs_urodynamics']:
        ...
        uds_icon="🔄"; uds_title="Pemeriksaan UDS Direkomendasikan"; uds_title_cl="#92400e"
        ...
        uds_icon="✅"; uds_title="Data NIVA-BOO Cukup Meyakinkan"; uds_title_cl="#14532d"
...
<div style="background:{bg_color}; border:2px solid {border_strong}; border-radius:16px; padding:2rem; text-align:center; box-shadow:0 2px 10px rgba(26,42,67,0.07);">
    <div style="font-family:'Fira Code',monospace; font-size:0.6rem; letter-spacing:0.2em; text-transform:uppercase; color:#8a9bb0; margin-bottom:0.5rem;">Probabilitas NIVA-BOO</div>
    ...
    <div style="font-family:'Fira Code',monospace; font-size:1.4rem; font-weight:600; color:{boo_color};">{prob_pct:.1f}%</div>
    <div style="margin:1rem 0; background:#e2e8f0; border-radius:8px; height:8px; overflow:hidden;">
        <div style="width:{display_pct:.1f}%; height:100%; background:{bar_gradient}; border-radius:8px;"></div>
    </div>
...
<div style="background:#ffffff; border:1.5px solid {border_color}; border-radius:14px; padding:1.3rem 1.5rem; margin-bottom:0.8rem; box-shadow:0 2px 8px rgba(26,42,67,0.06);">
    <div style="font-family:'Fira Code',monospace; font-size:0.58rem; letter-spacing:0.2em; text-transform:uppercase; color:#8a9bb0; margin-bottom:0.5rem;">Interpretasi NIVA-BOO</div>
    <div style="font-family:'Libre Baskerville',serif; font-size:1.15rem; color:{boo_color}; margin-bottom:0.5rem; font-weight:700;">{interp_title}</div>
...
<div style="background:#ffffff; border:1.5px solid {border_color}; border-radius:14px; padding:1.3rem 1.5rem; box-shadow:0 2px 8px rgba(26,42,67,0.06);">
    <div style="font-family:'Fira Code',monospace; font-size:0.58rem; letter-spacing:0.2em; text-transform:uppercase; color:#8a9bb0; margin-bottom:0.5rem;">Saran Klinis</div>
    <div style="font-family:'Libre Baskerville',serif; font-size:1rem; margin-bottom:0.5rem; font-weight:700; color:#1a2233;">{result['clinical_recommendation']}</div>
...
<div style="background:{uds_bg}; border:1.5px solid {uds_bd}; border-radius:12px; padding:1rem 1.2rem; display:flex; align-items:flex-start; gap:0.9rem; margin-bottom:0.8rem; box-shadow:0 2px 8px rgba(26,42,67,0.06);">
    ...
    <div>
        <div style="font-family:'Fira Code',monospace; font-size:0.58rem; letter-spacing:0.15em; text-transform:uppercase; color:{uds_cl}; margin-bottom:0.3rem;">Kebutuhan Urodinamika (UDS)</div>
...
""", unsafe_allow_html=True)
        ...
        if waveform_data is not None:
            ...
            fig2.patch.set_facecolor('#ffffff'); ax2.set_facecolor('#f5f7fa')
            ...
            plt.tight_layout(pad=0.3); st.pyplot(fig2, use_container_width=True); plt.close()

        st.markdown(f'<div style="font-family:\'Fira Code\',monospace; font-size:0.6rem; color:#8a9bb0; margin-top:0.4rem; text-align:center;">Sumber: {src_label}</div>', unsafe_allow_html=True)

    # ── Laporan ───────────────────────────────────────────────────────────────
    vol_display = f"{int(vol_prostat)} ml" if not np.isnan(vol_prostat) else "—"
    st.markdown(f"""
<div style="background:#ffffff; border:1.5px solid #d1d9e6; border-radius:16px; padding:2rem 2.5rem; margin-top:1.5rem; box-shadow:0 4px 16px rgba(26,42,67,0.08);">
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
        <div>
            <div style="font-family:'Fira Code',monospace; font-size:0.6rem; letter-spacing:0.2em; text-transform:uppercase; color:#8a9bb0; margin-bottom:0.3rem;">Laporan Analisis NIVA-BOO</div>
            <div style="font-family:'Libre Baskerville',serif; font-size:1.25rem; color:#1a2233; font-weight:700;">Non-Invasive Analysis for Bladder Outlet Obstruction</div>
        </div>
        <div style="text-align:right; font-size:0.8rem; color:#8a9bb0; background:#f5f7fa; border:1px solid #d1d9e6; border-radius:8px; padding:0.6rem 0.9rem;">
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:1.2rem; margin-top:1.5rem; padding-top:1.5rem; border-top:1.5px solid #eef1f6;">
        ...
        <div style="background:#f5f7fa; border:1px solid #d1d9e6; border-radius:10px; padding:1rem;">
            ...
            <div style="font-size:0.7rem; color:#8a9bb0; margin-top:0.2rem;">PVR: {pvr} mL &middot; Prostat: {vol_display}</div>
        </div>
    ...
    <div style="margin-top:1.2rem; padding-top:1rem; border-top:1px solid #eef1f6; font-size:0.68rem; color:#b0bdd0; font-family:'Fira Code',monospace;">
        ...
        Waveform: {src_label} &middot; ⚠️ Alat bantu skrining penelitian — bukan pengganti penilaian spesialis
    </div>
</div>
""", unsafe_allow_html=True)
...
⚠️ <b>PERHATIAN:</b> Analisis menggunakan Model A (Waveform Only) dengan AUC 0.52 — hampir setara random.
Hasil ini <b>tidak boleh digunakan</b> sebagai dasar keputusan klinis.
